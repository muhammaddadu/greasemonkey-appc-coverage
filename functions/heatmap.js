/**
 * Displays heatmap over github code
 */
	...
	for (var f = 0, max = this.coverage.length; f < max; ++f) {
		...
		if (url_path == this.path) {
			...
			for (var i = 0, maxI = lines.length; i < maxI; ++i) {
				var line = lines[i];
				if (line.hit > 0) {
					$('#LC' + line.line).css('background-color', '#'+rainbow.colourAt(line.hit));
				} else {
					$('#LC' + line.line).css('background-color', 'lightgrey');
				}
			}
		}
	}
 ...
 * Toggle Off
 */
heatmap.prototype.off = function() {
	for (var f = 0, max = this.coverage.length; f < max; ++f) {
		...
		if (url_path == this.path) {
			...
			for (var i = 0, maxI = lines.length; i < maxI; ++i) {
				var line = lines[i];
				$('#LC' + line.line).css('background-color', 'white');
			}
		}
	}
}
